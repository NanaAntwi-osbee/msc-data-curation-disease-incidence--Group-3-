...
## Data Provenance
* **Original Data Provider:** World Health Organization (WHO) Global Health Observatory (GHO)[cite: 1]
* **Portal Link:** [https://www.who.int/data/gho](https://www.who.int/data/gho?utm_source=copilot.com)[cite: 1]
* **Source Datasets:** 
  * Estimated number of people (all ages) living with HIV[cite: 1]
  * Estimated malaria incidence (per 1000 population at risk)[cite: 1]
  * Number of incident tuberculosis cases[cite: 1]
* **Date of Retrieval:** September 24, 2026[cite: 1]
* **Original Filenames:** `HIV.xlsx`, `Malaria.xlsx`, `TUBERCULOSIS.xlsx`[cite: 1]

## Scope & Coverage
* **Geographic Scope:** Sub-Saharan Africa. 
* **Country-Selection Rule:** Selection is strictly based on the World Bank's regional classification for Sub-Saharan Africa[cite: 1]. Non-sovereign entities and regional aggregates were explicitly excluded to maintain sovereign country-level analysis[cite: 1].
...
* **Temporal Scope:** 2000 to 2025[cite: 1].
...
* **Validation & Cleaning:** New derived variables (`MetricType` and `Denominator`) were added to explicitly distinguish raw incidence counts from incidence rates[cite: 1]. Columns with high sparsity that did not align with the core indicators were dropped.
* **Software Used:** Python 3, Pandas (via `pd.read_excel` and `pd.concat`).

## Data Quality & Limitations
* **Missing-Data Conventions:** Empty numeric observations (e.g., missing uncertainty bounds) were explicitly converted to the string `"NA"` to distinguish missing data from true zero values[cite: 1].
...
* **Suggested Citation:** Armoh, F., Takyi, B., Annan, F., Ennibil, J., & Agbemaka, J. K. M. (2026). *Longitudinal Disease Incidence in Sub-Saharan Africa: Curated Indicators for HIV, Malaria, and Tuberculosis*. Version 1.0. University of Cape Coast. Repository: https://github.com/NanaAntwi-osbee/msc-data-curation-disease-incidence--Group-3-

---

## Reproducibility & Metadata Examples
**Reproduction Instructions:** 
To reconstruct the processed CSV, download the raw indicators from the [WHO Global Health Observatory](https://www.who.int/data/gho?utm_source=copilot.com)[cite: 1]. Save the unmodified Excel workbooks in `data/raw/`[cite: 1]. Execute `code/data_preparation.py` using Python to merge the sets, filter the 47 Sub-Saharan countries, and standardize the metrics[cite: 1].
